...
//				$needs      = $requirements[0];
			...
			wp_send_json_success( sprintf( __( '%d Users earned this badge.', 'mycred' ), count( $userIds ) ) );
		}
		...
		public function register_post_type() {
			...
			$labels = array(
				'name'              => _x( 'Categories', 'taxonomy general name', 'mycred' ),
				'singular_name'     => _x( 'Category', 'taxonomy singular name', 'mycred' ),
				'search_items'      => __( 'Search Categories', 'mycred' ),
				'all_items'         => __( 'All Categories', 'mycred' ),
				'parent_item'       => __( 'Parent Category', 'mycred' ),
				'parent_item_colon' => __( 'Parent Category:', 'mycred' ),
				'edit_item'         => __( 'Edit Category', 'mycred' ),
				'update_item'       => __( 'Update Category', 'mycred' ),
				'add_new_item'      => __( 'Add New Category', 'mycred' ),
				'new_item_name'     => __( 'New Category Name', 'mycred' ),
				'menu_name'         => __( 'Category', 'mycred' ),
			);
		...
		function my_restrict_manage_posts() {
			...
			global $typenow;
			if ( $typenow == 'mycred_badge' ) {

				$filters = array( 'mycred_badge-category' );
		...
		public function adjust_column_headers( $defaults ) {
			...
			$defaults['mycred_badge-category'] = 'Categories';
		...
		public function adjust_column_content( $column_name, $post_id ) {
			...
			} elseif ( $column_name == 'mycred_badge-category' ) {
				$categories = get_the_terms( $post_id, 'mycred_badge-category' );
				$cats       = array();
		...
		public function publishing_actions() {
			...
			if ( ! isset( $post->post_type ) || $post->post_type != 'mycred_badge' ) {
				return;
			}
		...
		public function metabox_badge_requirements( $post ) {
			...
			<style type="text/css">
				#mycred-remove-badge-connections {
					float: right;
				}
				...
				table#setup-badge-reqs {
					width: 100%;
				}

				table#setup-badge-reqs td.type {
					width: 25%;
				}

				table#setup-badge-reqs td.for {
					width: 5%;
				}

				table#setup-badge-reqs td.reference {
					width: 50%;
				}

				table#setup-badge-reqs td.amount {
					width: 10%;
				}

				table#setup-badge-reqs td.sum {
					width: 10%;
				}
		...
		public function metabox_badge_images( $post ) {
			...
			<style type="text/css">
				#mycred_badge_images .inside, #mycred_badge_images .inside p {
					margin: 0;
					padding: 0;
				}
				...
				#image-wrapper > div.inner-box .thumb {
					display: block;
					width: 100px;
					height: 100px;
					float: left;
				...
				#image-wrapper > div.inner-box .thumb img {
					max-width: 100px;
					height: auto;
					margin: 0 auto;
				}

				#image-wrapper > div.inner-box .thumb p {
					line-height: 100px;
					text-align: center;
					color: #ccc;
				}

				#image-wrapper > div.inner-box .details {
					margin-left: 125px;
					padding: 12px;
				}

				#image-wrapper > div.inner-box .details p strong {
					display: block;
				}

				#image-wrapper > div.inner-box .details p.desc {
					margin-bottom: 12px;
				}
			</style>
		...
		public function after_general_settings() {
			...
			<h4>
				<div class="icon icon-hook icon-active"></div>
				<label>Badges</label></h4>
						...
						<li>
							<select name="<?php echo $this->field_name( 'buddypress' ); ?>" id="<?php echo $this->field_id( 'buddypress' ); ?>">
								...
								foreach ( $buddypress_options as $location => $description ) {
									...
									if ( isset( $settings['buddypress'] ) && $settings['buddypress'] == $location ) {
										echo ' selected="selected"';
									}
						...
						<li>
							<select name="<?php echo $this->field_name( 'bbpress' ); ?>" id="<?php echo $this->field_id( 'bbpress' ); ?>">
								...
								foreach ( $bbpress_options as $location => $description ) {
									echo '<option value="' . $location . '"';
									if ( isset( $settings['bbpress'] ) && $settings['bbpress'] == $location ) {
										echo ' selected="selected"';
									}
									echo '>' . $description . '</option>';
								}
